...
export function handleMessages(sock: ModifiedSock, websocket: WebSocket, fallback: (uuid: string) => any, uuid: string) {
  return async (m: { messages: WAMessage[]; type: MessageUpsertType; requestId?: string }) => {
    ...
    father: for (const trigger of triggers) {
      ...
      if (trigger.TextTrigger.length > 0) {
        ...
        for (const textTrigger of trigger.TextTrigger) {
          ...
          const passedTextTrigger = textTriggerTest(
            textTrigger.type as any,
            textTrigger.text,
            m.messages[0].message?.conversation as string
          );
    ...
    await prisma.triggerLog.create({
      data: { triggerId: responses.ResponseTriggerRelation[0].triggerId },
    });
    for (const response of responses.ResponseTriggerRelation) {
      ...
      await prisma.responseLog.create({
        data: { responseId: response.responseId },
      });
    }
  };
}
