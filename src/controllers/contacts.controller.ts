...
export const contactsController: FastifyPluginAsync = async (fastify: FastifyInstance) => {
  fastify.get<{
    ...
    const formattedContacts = contacts.map((contact) => ({
      ...contact,
      clusterName: contact.Cluster ? contact.Cluster.name : 'Sem grupo',
    }));
    const total = await prisma.contacts.count({ where: { userId: uuid } });
    return { total, contacts: formattedContacts };
    return reply.send(formattedContacts);
  });
  fastify.get('/contacts-cluster', async (req, reply) => {
    const uuid = req.me.id;
    const client = req.clients.find((c) => c.clientUUid === uuid);
    if (!client) {
      // return []
    ...
    const formattedData = data.map((cluster) => {
      return { ...cluster, contacts: cluster.Contacts };
    });
  ...
  fastify.post<Body<{ contactIds: Contacts[]; message: string }>>('/contacts/send-message', async (req, reply) => {
    const uuid = req.me.id;
    const client = req.clients.find((c) => c.clientUUid === uuid);
    if (!client) {
      // return []
